...
from .titlepredictor import jobtitleclassify,experiencemapper
from .utils import handle_pdf,clean_text,process_file
from .tasks import create_ner_pool_task,send_verification_email_task
...
def jobseeker_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                create_ner_pool_task.delay(job_seeker.id)
...
import os
import tempfile
from .descriptionhandler import DescriptionHandler  # adjust to wherever that class actually lives

@login_required
def extract_job_description(request):
    if request.method != 'POST' or not request.FILES.get('job_description_file'):
        return JsonResponse({'error': 'No file provided'}, status=400)

    f = request.FILES['job_description_file']
    ...
    try:
        handler = DescriptionHandler.from_file_path(tmp_path)
        data = handler.regex_extraction()
    finally:
        os.remove(tmp_path)
...
def start_test(request, uidb64, token, job_id):
   ...
   #category=job.job_title
    ...
    questions_objective = Question.objects.filter(category=category_mapper,question_type='objective', difficulty=difficulty_mapper).order_by('?').distinct()[:8]
    questions_mcq = Question.objects.filter(category=category_mapper, question_type='mcq', difficulty=difficulty_mapper).order_by('?').distinct()[:8]
    questions_code = Question.objects.filter(category=category_mapper, question_type='code', difficulty=difficulty_mapper).order_by('?').distinct()[:4]
