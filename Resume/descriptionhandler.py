...
class DescriptionHandler:
    ...
    def __init__(self, job_id=None):
        self.job_id = job_id
        self.job = Job.objects.filter(id=self.job_id).first() if job_id else None
        self.description_text = ""

    @classmethod
    def from_file_path(cls, file_path):
        """
        Build a handler directly from a file path, without needing a saved
        Job row. Useful for pre-submit "extract and prefill" flows where the
        file hasn't been attached to a Job yet.
        """
        handler = cls(job_id=None)
        handler.description_text = handler._read_file(file_path)
        return handler

    @classmethod
    def from_text(cls, text):
        """Build a handler directly from raw text (e.g. already-read content)."""
        handler = cls(job_id=None)
        handler.description_text = text
        return handler
    ...
    def extract_pdf(self):
        ...
        self.description_text = self._read_file(file_path)
